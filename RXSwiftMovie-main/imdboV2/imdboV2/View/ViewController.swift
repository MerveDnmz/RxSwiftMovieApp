...
class ViewController: UIViewController ,UITableViewDelegate, UISearchBarDelegate{
    ...
    private func setupBindings()
    {
        ...
        movieVM
            .movies
            .bind(to: tableView.rx.items(cellIdentifier: "ListCell",
                                         cellType: MovieSearchResultCell.self)) { (row,item,cell) in
                cell.item = item }.disposed(by: disposeBag)
        ...
        tableView.rx.modelSelected(ListResult.self).subscribe(onNext: { item in
            ...
            self.performSegue(withIdentifier: "MovieListToMovieDetailVC", sender: nil)
            }).disposed(by: disposeBag)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "MovieListToMovieDetailVC" {
